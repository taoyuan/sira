 ...
 * @examples
 *
 * sira.phase(nao.boot.database({ driver: 'redis' });
 *
 * sira.phase(nao.boot.database({
 *      mysql: {
 *          driver: 'mysql'
 *      },
 *      redis: {
    ...
    return function () {
        ...
        _.forEach(schemap, function (v, k) {
            ...
            schema[k].on('define', function(m, name, prop, sett) {
                sira.models[name] = middistModelHooks(m);
                if (conf.backyard) {
...
//            }
            ...
            _.forEach(models, function (model) {
                if (model.setup) model.setup(model, sira);
            });
        ...
        if (settings.autoupdate) {
            sira.__postprocessors.add(function () {
