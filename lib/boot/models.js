"use strict";

var path = require('path');
var needs = require('needs');
var _ = require('lodash');
var i8n = require('inflection');


/**
 * Models initialization phase.
 *
 * @examples
 *
 * noa.phase(nao.boot.models('models');
 *
 * @param dir models directory.
 * @returns {Function}
 */

module.exports = function (dir) {
    dir = dir || path.join(__dirname, '..', 'models');

    return function () {
        var registry = require('../registry')(this.__definitions);

        var defs = needs(path.resolve(dir), { pattern: '**/*.js', excludes: '**/*.setup.js' });
        var setups = needs(path.resolve(dir), { patterns: '**/*.setup.js' });


        _.forEach(defs, function (def, name) {
            var modelName = def.name || i8n.camelize(name);
            registry.define(modelName, typeof def === 'function' ? def() : def, findSetup(modelName));
        });

        function findSetup(name) {
            name = name.toLowerCase() + '.setup';
            var found = _.find(Object.keys(setups), function (key) {
                return key.toLowerCase() === name;
            });
            return found ? setups[found] : null;
        }
    }
};