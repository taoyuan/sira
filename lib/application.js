...
Application.prototype.exports = function(name, handler) {
    if (!handler) return this.__remotes.exports[name];

    var _handler = this.__remotes.exports[name] || { name: name, app: this };

    if (typeof handler === "function") {
        handler.call(_handler, this);
    } else {
        _.assign(_handler, handler);
    }

    return this.__remotes.exports[name] = _handler;
};
...
//    this.__middist.handle(ctx, function (err, ctx) {
//        if (err && !cb) throw err;
//        cb && cb(err, ctx && ctx.result);
//    });
};
