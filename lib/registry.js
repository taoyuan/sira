...
var debug = require('debug')('noa:registry');
    ...
    registry.publish = function (db, defs) {
        ...
        function publishRelations(model, relations) {
            ...
            _.forEach(relations, function (relation, name) {
                ...
                if (!targetModel) {
                    throw new Error(util.format('Unknown target model `%s` to build relationship %s.%s %s',
                        target, model.modelName, relation.type, target));
                }
